...
class UserResource extends JsonResource
{
     ...
     */
    private function resolveDepartment(): ?array
    {
        $department = $this->studentProfile?->program?->department
            ?? $this->studentProfile?->department
            ?? $this->facultyProfile?->department;
    ...
    {
        $coordinator = $this->activeInternship?->coordinator;

        if (! $coordinator) {
            return 'N/A';
        }

        if ($this->studentProfile && ! \App\Support\DepartmentScope::facultyMatchesStudent($coordinator, $this->studentProfile)) {
            return 'N/A';
        }
    ...
    {
        $faculty = $this->activeInternship?->faculty;
        if ($faculty && $this->studentProfile && ! \App\Support\DepartmentScope::facultyMatchesStudent($faculty, $this->studentProfile)) {
            $faculty = null;
        }

        if ($faculty?->facultyProfile) {
            $p = $faculty->facultyProfile;

            return trim("{$p->first_name} {$p->last_name}") ?: 'Not Assigned';
        }

        $resolved = app(FacultySectionAssignmentService::class)
            ->resolveFacultyForProfile($this->studentProfile);

        if ($resolved?->facultyProfile) {
            return trim("{$resolved->facultyProfile->first_name} {$resolved->facultyProfile->last_name}") ?: 'Not Assigned';
        }
    ...
    {
        return AvatarUrl::fromPath($path);
    }
}
